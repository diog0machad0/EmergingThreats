...
JOES Threat Intelligence is a monolithic Python application with a clear separation of concerns across modules. This page describes the system architecture, data flow, and key design decisions.
...
A trailing "Recommended Posts" / "Related Articles" section is then stripped. Many sites render these link lists inside the article container, where trafilatura keeps them even with `favor_precision=True`, and they carry other articles' headlines. Left in, a MikroTik RouterOS write-up ended with headlines naming NetScaler and JFrog Artifactory, which the customer tech matcher and the CVE pairing then attributed to that article. The cut is only applied to a marker found in the last 30% of the text, so a mid-article "read more" cannot truncate the body.
...
Every `load_config()` merges feeds from `data/config.json.example` into `config.json` so new example feeds reach existing installations. The merge skips an example feed when **either** its URL or its name is already configured. Both keys are required: matching on URL alone meant that correcting a feed's URL left the example's original URL unmatched, so the broken endpoint was re-added as a duplicate on the next load and no URL edit could ever stick.
...
## JOES Threat Intelligence operational extensions

This fork adds analyst workflows on top of the core pipeline:

```
Summarize (+ TI match hook)
       │
       ├─► emerging_threat_analyses   → /emerging-threats (Threat Intelligence + triage)
       ├─► vulnerability_analyses     → /emerging-threats (Vulnerabilities + triage)
       ├─► advisories (SQLite index)  → Scriba PDF under ../scriba/
       └─► apt_ioc_exports            → MISP (optional)
```

| Module | Role |
|---|---|
| `emerging_threats.py` | Customer footprint matching + LLM outlook |
| `vulnerability_threats.py` | CVE + tech-stack hits |
| `tech_matching.py` | Shared tech-stack matcher used by both queues |
| `advisory_generator.py` | LLM advisory markdown + Scriba PDF build |
| `misp_client.py` / `apt_*` / `ioc_extractor.py` | APT consolidation and MISP export |
| `slack_client.py` | Advisory PDF distribution to analyst-selected Slack channels |
| `advisory_notification.py` | Security Vulnerability Notification template — LLM fills fields, code renders layout |
| `llm_client.py` | Also supports **OpenRouter** (OpenAI-compatible) |

Triage (`pending` / `triaged`) filters analyst queues without deleting underlying analyses. See feature docs under `docs/features/emerging-threats-ops.md`, `advisories.md`, `apt-misp.md`, and `slack-distribution.md`. Monorepo install notes: `../INSTALL.md`.
