...
> **JOES Threat Intelligence monorepo:** If you cloned `emergingJoes`, start at the repo root
> [`INSTALL.md`](../../INSTALL.md) and [`AGENT_HANDOFF.md`](../../AGENT_HANDOFF.md).
> The steps below remain valid inside the `EmergingJoes/` directory.

This guide walks you through installing JOES Threat Intelligence, configuring your API keys, and running your first intelligence pipeline.
...
| LLM API key | — | OpenAI, Anthropic, **or** OpenRouter (Settings) |
| OpenAI key | — | Still needed for embeddings / intelligence search when using those features |
| Malpedia API key | — | Optional, for research article ingestion |
| Scriba system libs | — | Optional until you generate PDF advisories (see root INSTALL.md) |
...
If you prefer running JOES Threat Intelligence in a container, Docker is the quickest way to get started.
...
On startup, JOES Threat Intelligence will:
...
??? warning "Port already in use"
    JOES Threat Intelligence automatically finds a free port starting from 5000. If you see connection errors, check that no other process is using the selected port.
