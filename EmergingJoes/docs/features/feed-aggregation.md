...
JOES Threat Intelligence automatically collects threat intelligence articles from RSS/Atom feeds and the Malpedia research library. A built-in LLM relevance filter ensures only genuine threat intelligence enters the database.
...
JOES Threat Intelligence ships with 13 curated cybersecurity feeds:
...
Not all articles from security feeds are threat intelligence. Many are product announcements, opinion pieces, or general IT news. JOES Threat Intelligence uses an LLM to classify relevance:
...
[Malpedia](https://malpedia.caad.fkie.fraunhofer.de/) is a curated repository of threat research maintained by Fraunhofer FKIE. JOES Threat Intelligence integrates with it as an additional article source.
