...
JOES Threat Intelligence uses SQLite with WAL (Write-Ahead Logging) mode for persistent storage. The database file is `threatlandscape.db` in the project root.
...
| `fetched_date` | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | When JOES Threat Intelligence ingested the article |
...
## Operational tables (JOES Threat Intelligence)

### `customers`

Analyst-defined customer footprints used for TI / vulnerability matching.
...
| `business` | TEXT | |
| `country` | TEXT | |
| `known_affiliates` | TEXT | JSON array |
| `known_tech_stack` | TEXT | JSON array |

### `emerging_threat_analyses`

| Column | Type | Description |
|---|---|---|
| `article_id` | INTEGER UNIQUE FK | |
| `hits_json` | TEXT | Customer match hits |
| `triage_status` | TEXT | `pending` (default) or `triaged` |
| `triaged_date` | TIMESTAMP | Set when triaged |

### `vulnerability_analyses`

Same shape as emerging threats (`hits_json` with CVE + tech fields) plus `triage_status` / `triaged_date`.

### `advisories`

Index of generated Scriba reports (not full body storage).

| Column | Type | Description |
|---|---|---|
| `kind` | TEXT | `vulnerability` \| `threat_intelligence` \| `general` |
| `article_id` / `cve_id` | | Linkage for “advisory was made” |
| `stem` | TEXT UNIQUE | Filename stem |
| `pdf_filename` | TEXT | Served via `/api/advisories/pdf/...` |

### `apt_ioc_exports`

MISP export attempts per article / APT (`status`, event ids, galaxy tag, IOC JSON).

```
customers ────────── (matched in hits_json)
articles 1──────────1 emerging_threat_analyses
articles 1──────────1 vulnerability_analyses
articles 1──────────* advisories
articles 1──────────1 apt_ioc_exports
```
