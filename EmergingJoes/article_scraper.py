...
# Headings that end an article and begin a list of links to other articles.
# Many sites render these inside the article container, so trafilatura keeps
# them even with favor_precision. Leaving them in attributes other articles'
# products and CVEs to this one, which then reach the customer tech matcher and
# the CVE pairing as if this article had mentioned them.
_TRAILING_BOILERPLATE_RE = re.compile(
    r"\n[^\n]{0,60}?\b("
    r"recommended (posts|reading|articles|for you)"
    r"|related (posts|reading|articles|stories|content|news)"
    r"|you might (also )?(be interested|like|enjoy)"
    r"|more (from|like this|stories)"
    r"|popular (posts|articles|stories)"
    r"|latest (posts|articles|news)"
    r"|read (more|next)"
    r"|also on"
    r"|trending (now|stories)"
    r")\b[^\n]{0,60}\n",
    re.I,
)

# Only trim inside the tail. A mid-article "read more" is part of the story.
_TRAILING_SEARCH_FRACTION = 0.7


def _strip_trailing_boilerplate(text):
    """Cut a trailing related-links section from extracted article text.

    Returns the text unchanged when no marker is found in the tail, so a
    false positive cannot silently truncate the body of an article.
    """
    if not text:
        return text
    start = int(len(text) * _TRAILING_SEARCH_FRACTION)
    match = _TRAILING_BOILERPLATE_RE.search(text, start)
    if not match:
        return text
...
def _fetch_and_extract(url):
    ...
    if text and len(text) > 100:
        return _strip_trailing_boilerplate(text)
