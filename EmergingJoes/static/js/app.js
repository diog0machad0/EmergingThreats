...
function createArticleCard(article) {
    ...
    card.innerHTML = `
        ...
        <div class="article-card-expanded">
            ...
            ${article.summary_text ? `<p class="llm-disclaimer">This summary was generated by JOES Threat Intelligence using large language models (LLMs). LLMs can make mistakes. Always verify important information against the original source.</p>` : ''}
        </div>
    `;
...
// The app shell scrolls .content-area rather than the window, so listen there
// and fall back to the window if the shell is ever absent.
(function bindInfiniteScroll() {
    ...
    const onScroll = () => {
        ...
        const contentHeight = scroller ? scroller.scrollHeight : document.body.offsetHeight;

        if (viewportBottom >= contentHeight - 200) {
            currentPage++;
            loadArticles(true);
        }
    };
...
function saveSettings() {
    ...
    const settings = {
        ...
        openai_model: model ? model.value : 'gpt-4.1-mini',
        ...
        misp_enabled: document.getElementById('misp-enabled') ? document.getElementById('misp-enabled').checked : true,
        misp_url: document.getElementById('misp-url') ? document.getElementById('misp-url').value : '',
        misp_api_key: document.getElementById('misp-api-key') ? document.getElementById('misp-api-key').value : '',
        misp_verify_ssl: document.getElementById('misp-verify-ssl') ? document.getElementById('misp-verify-ssl').checked : false,
        slack_enabled: document.getElementById('slack-enabled') ? document.getElementById('slack-enabled').checked : false,
        slack_bot_token: document.getElementById('slack-bot-token') ? document.getElementById('slack-bot-token').value : '',
    };
...
async function testMispConnection() {
    const status = document.getElementById('misp-test-status');
    if (status) {
    ...
    try {
        ...
        const data = await res.json();
        if (status) {
            status.textContent = data.message || (data.connected ? 'Connected' : 'Not connected');
            status.style.color = data.connected ? 'var(--accent-green)' : 'var(--accent-red)';
        }
    } catch (e) {
        if (status) {
...
async function testSlackConnection() {
    const status = document.getElementById('slack-test-status');
    const field = document.getElementById('slack-bot-token');
    if (status) {
        status.textContent = 'Testing...';
        status.style.color = 'var(--text-secondary)';
    }
    try {
        // Send whatever is currently typed so the token can be verified before
        // it is saved. The server does not persist it.
        const res = await fetch('/api/slack/status', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token: field ? field.value.trim() : '' }),
        });
        const data = await res.json();
        if (status) {
            let text = data.message || (data.connected ? 'Connected' : 'Not connected');
            if (data.connected) {
                text += ` — ${data.channel_count} channel${data.channel_count === 1 ? '' : 's'} available`;
            }
