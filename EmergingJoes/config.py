...
def load_config():
    ...
    if not os.path.exists(CONFIG_PATH):
        ...
        # Matching on name as well as URL is what makes a corrected feed URL
        # stick: keyed on URL alone, editing a feed leaves the example's
        # original URL unmatched, so it was re-added as a duplicate on the very
        # next load and the broken endpoint came back for good.
        existing_urls = {f["url"] for f in config.get("feeds", [])}
        existing_names = {
            (f.get("name") or "").strip().lower() for f in config.get("feeds", [])
        }
        new_feeds = [
            f for f in _load_example_feeds()
            if f["url"] not in existing_urls
            and (f.get("name") or "").strip().lower() not in existing_names
        ]
        if new_feeds:
    ...
    env_slack = os.environ.get("SLACK_BOT_TOKEN")
    if env_slack:
        config["slack_bot_token"] = env_slack
