...
IRRELEVANT — exclude if the article is ONLY about:
  ...
  generic cybersecurity trends/surveys/predictions without a specific active threat or incident,
  vendor thought leadership or best-practice guides without reporting an actual attack or campaign.

IMPORTANT: When in doubt about active threats, classify as IRRELEVANT for this pipeline.
We only want articles reporting real adversary activity, not industry commentary.
...
def summarize_pending(limit=10, article_ids=None):
    ...
    for article in articles:
        ...
        if result:
            ...
            try:
                from emerging_threats import analyze_and_store_customer_threats
                analyze_and_store_customer_threats(article_id, title, content)
            except Exception as e:
                logger.warning(f"  Emerging threat analysis failed for article {article_id}: {e}")

            try:
                from vulnerability_threats import analyze_and_store_vulnerability_threats
                analyze_and_store_vulnerability_threats(article_id, title, content)
            except Exception as e:
                logger.warning(f"  Vulnerability analysis failed for article {article_id}: {e}")

            try:
                from apt_ioc_processor import process_apt_iocs_for_article
                process_apt_iocs_for_article(
                    article_id=article_id,
                    title=title,
                    content=content,
                    tags=result.get("tags"),
                ...
                logger.warning(f"  APT IOC / MISP export failed for article {article_id}: {e}")
